...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(document).ready(function() {
	  ...
	  Height.init('[am-profile~=same-height]', '[am-profile~=same-height] > div');
	
	  // Filters
	  Filter.sort('[am-filter~=link]', '[am-profile~=collection] > li', '[am-filter~=list]', '[am-filter~=toggle]');
	  Filter.init(null,'[am-filter~=toggle]');
	
	  // Social Sharing
	  SocialSharing.init('[am-profile~=tile-sharing]', '[am-social~=sharing-icons]');
	
	  // Badge selection
	  $('[am-profile~=badge-link]').click(function(e) {
	    var badgeTitle = $(this).attr('data-title'),
	        badgeLevel = $(this).attr('data-level'),
	        badgeDate = $(this).attr('data-date'),
	        badgeDetails = $(this).attr('data-details'),
	        badgeColour = $(this).attr('data-colour'),
	        badgeImage = $(this).attr('data-badge');
	    ...
	    $('[am-badge~=image]').attr('src', '../assets/img/badges/' + badgeImage);
	    $('[am-badge~=title]').html(badgeTitle);
	    $('[am-badge~=level]').html(badgeLevel);
	    $('[am-badge~=date]').html(badgeDate);
	    $('[am-badge~=details]').html(badgeDetails);
	  ...
	  // Show button if you have more than 18 badges on page start
	  if( $('[am-profile~=badge-link]:visible').length >= 18) {
	    $('[am-badges~=show-more]').show();
	  }
	
	  // Show button if you have more than 18 badges on filter click
	  $('[am-filter~=link]').on('click', function(){
	    if($('[am-profile~=badge-link]:visible').length  >= 18) {
	      $('[am-badges~=show-more]').show();
	    } else {
	      $('[am-badges~=show-more]').hide();
	    }
	  });
...
/***/ })
/******/ ]);
//# sourceMappingURL=badge-hub.min.js.map
//# sourceMappingURL=badge-hub.min.js.map
